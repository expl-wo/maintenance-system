...
var PDFViewerApplication = {
  ...
  setTitleUsingUrl: function setTitleUsingUrl(url) {
    ...
    let realFileName = this.getFileName();
    if(realFileName){
      title = realFileName;
    }
    this.setTitle(title);
  ...
  getFileName(){
    try{
      let splitStr = 'fileName=';
      let index = window.location.href.indexOf(splitStr);
      if(index>0){
        return decodeURIComponent(window.location.href.substr(index + splitStr.length));
      }else{
        return '';
      }
    }catch (e){
