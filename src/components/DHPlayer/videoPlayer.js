...
(function () {
    window.dhPlayerControl = Object.assign({
        ...
        DHPlayerVersion: '',
        pkgDHPlayerVerion: [2310201130, 2310201130], // 配套的插件版本号
        isPIframe: false,
    ...
    function socketPort() {
        ...
        function eerror(evt) {
            ...
            if(that.setting.isResetConnect) {
                eLen++
                var port = evt.target.url.split(':').length == 3 ? evt.target.url.split(':')[2].replace('/', '') : '80'
                map['ws' + port].close()
        ...
        if (data.info && typeof data.info.hwnd === 'number') {
            ...
            switch (data.method) {
                case 'window.loginServer.notify':
                    ...
                    } else if(data.info.result === -6) {
                        hwndInfo.createSuccess && hwndInfo.createSuccess(getVersionInfo())
                    } else {
                        hwndInfo.createError && hwndInfo.createError({
                            code: 1004,
                            data: null,
                            message: '插件登录失败，请检查登录信息是否正确',
                            success: false
                        })
                    }
                    break;
                    ...
                    domSize = [
                        {
                            left: domSize[0].left,
                            top: domSize[0].top,
                            width: domSize[0].width,
                            height: domSize[0].height,
                        }
                    ]
                    if(pOutContent) {
                        ...
                        domSize[0].right = domSize[0].right + domSize[0].width
                    }
