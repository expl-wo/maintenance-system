<template>
  <div class="uploadclass">
    ...
    >
      ...
      <el-button v-if="!simple && canDelete" class="deleteBtn" @click="handleDelete" type="danger">删除</el-button>
      <template #tip>
    ...
    <xui-download v-if="!simple && fileName" :href="downloadFileUrl" :fileName="fileName"></xui-download>
...
import {initMultipart, completeMultipart} from "@/_public/api/file";
import {getFileType} from "@/_public/utils";
import Constants from '@/_public/utils/constants'
import {ElMessageBox, useFormItem} from "element-plus";
import {ElMessage} from 'element-plus';
import {addFilePrev} from '@/_public/utils/index'
...
export default {
  ...
  setup(props, {emit}) {
    ...
    const downloadFileUrl = computed(() => {
      return addFilePrev(props.fileUrl);
    });
    ...
    //分片上传完成合并
     const composeFile = async (uploadId, fileName, chunkSize, fileSize, contentType, uploadName) =>{
      ...
      if(response.code === Constants.respCode.success){
        ...
        emit('uploadSuccess', fileName, response.data)
        uploadRef.value?.clearFiles()
        nextTick(() => {
          formItem?.validate('');
        })
