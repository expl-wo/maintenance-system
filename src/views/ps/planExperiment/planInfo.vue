...
export default {
  ...
  methods: {
      ...
      this.$confirm('确认全部提交审批？', '提示', {
        ...
        this.tableData.forEach(item=>{
          if(item.id && item.confirmStatus === 1){
            nodeId = item.nodeId
            let kvo = {}
            kvo.k = item.productPlanId
            kvo.v = ''
            idKvArray.push(kvo)
          }
        })
        planWeek.applyWeekPlan({nodeId:nodeId,condition:idKvArray}).then(res=>{
    ...
    handlePass(){
      ...
      planWeek.approvalPlan({nodeId:nodeId,condition:passInfo,approveStatus:this.$constants.isPass.yes}).then(res=>{
        if(res.err_code===10000){
          this.queryTableDataParam()
    ...
    handleDelete(){
      ...
      planWeek.delPlan({ids:delId}).then(res=>{
        if(res.err_code===10000){
          this.queryTableDataParam()
