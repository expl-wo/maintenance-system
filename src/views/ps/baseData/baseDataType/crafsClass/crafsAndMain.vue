<template>
  ...
  <div class="app-container app-containerC">
    <div class="filter-container searchCon">
      <el-form :inline="true" :model="listQuery" class="demo-form-inline demo-form-zdy">
        <el-form-item label="主材类型名称" >
          <el-input v-model="listQuery.mainTypeName" placeholder="主材类型名称" style="width: 180px;" class="filter-item" size="small" clearable/>
        </el-form-item>
        <el-form-item >
    ...
    <crafs-and-main-add-or-update-dialog @refresh="onBtnQuery" ref="crafsAndMainAddOrUpdateDialogRef"></crafs-and-main-add-or-update-dialog>
  </div>
</template>
...
// 辅材类型查询
import { deleteMain, getCrafsAndMain} from '@/api/eqpLedger'
import Pagination from "@/components/Pagination/index";
import Constants from "@/utils/constants";

import crafsAndMainAddOrUpdateDialog from "@/views/ps/baseData/baseDataType/crafsClass/crafsAndMainAddOrUpdateDialog";

export default {
  components: {crafsAndMainAddOrUpdateDialog,Pagination },
  data() {
    return {
      total: 0, // 角色列表表格总条数
      listQuery: { // 查询条件
  ...
  methods: {
    ...
    onQuery() {
      this.tableData = []
      getCrafsAndMain(this.listQuery).then(response => {
    ...
    handleAdd(){
      this.$refs.crafsAndMainAddOrUpdateDialogRef.init();
    },
    handleEdit(rowData){
      this.$refs.crafsAndMainAddOrUpdateDialogRef.init(rowData);
    },
