<template>
  <el-dialog v-dialogDrag  appendToBody :title="model.id? '编辑': '新增'" width="600px" v-model="dialogVisible" modal>
    <el-form :model="model" ref="form" :rules="rules" label-width="140px">
      <el-row>
        <el-col :span="24">
          <el-form-item prop="value" label="辅材类型名称：" s>
...
import { saveEquipment} from "@/api/eqpLedger";


export default {
  name: 'crafsEquipmentAddOrUpdateDialog',
  data() {
    return {
      initModel: {
       equipmentTypeName:'',
  ...
  methods: {
    ...
    handleSubmit(){
      this.$refs.form.validate(val=>{
       ...
       saveEquipment(this.model).then(resoponse=>{
