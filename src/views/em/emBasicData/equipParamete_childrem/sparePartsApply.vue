<template>
  <div class="app-container">
    ...
    <el-table stripe  highlight-current-row  :data="tableData" :header-cell-style="{'text-align':'center'}" :cell-style="{'text-align':'center'}"
      ...
      <el-table-column label="配件名称" :width="150">
        <template  #default="scope">
          <el-input v-model="scope.row.spName" :disabled="scope.row.id != null" size="mini"></el-input>
        </template>
      </el-table-column>
      <el-table-column label="配件编码" :width="150">
        <template  #default="scope">
          <el-input v-model="scope.row.materialCode" :disabled="scope.row.id != null" size="mini"></el-input>
        </template>
      </el-table-column>
      <el-table-column label="配件型号" :width="150">
        <template  #default="scope">
          <el-input v-model="scope.row.spModel" :disabled="scope.row.id != null" size="mini"></el-input>
        </template>
      </el-table-column>
      <el-table-column label="配件类型" :width="200" >
        <template  #default="scope">
          <el-radio-group v-model="scope.row.spType"  size="mini">
      ...
      <el-table-column label="单位" :width="90">
        <template  #default="scope">
          <el-input v-model="scope.row.unit" :disabled="scope.row.id != null" size="mini"></el-input>
        </template>
      </el-table-column>
      <el-table-column label="采购数量" :width="120">
        <template  #default="scope">
          <el-input-number v-model="scope.row.quantity" :controls="false" :min="1" size="mini" style="width: 80px;">
          </el-input-number>
        </template>
      </el-table-column>
      <el-table-column label="单价" :width="120">
        <template  #default="scope">
          <el-input-number v-model="scope.row.price" :controls="false" :min="1" size="mini" style="width: 80px;">
          </el-input-number>
        </template>
      </el-table-column>
      <el-table-column label="指定厂家">
        <template  #default="scope">
          <el-input v-model="scope.row.factory" size="mini"></el-input>
        </template>
      </el-table-column>
      <el-table-column label="期望到货时间" :width="200">
        <template  #default="scope">
          <el-date-picker v-model="scope.row.expectedArrivalTime" style=" width: 130px;" size="mini" align="right" type="date"
      ...
      <el-table-column :width="50" label="操作" fixed="right">
        <template  #default="scope">
          <el-button type="danger" size="mini" icon="el-icon-delete" @click.native.prevent="deleteRow(scope.$index, tableData)">
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!--选择设备-->
    <el-dialog v-draggable :close-on-click-modal="false" title="选择设备" :visible.sync="dialogEquipFormVisible"
      ...
      <div class="filter-container searchCon">
        <el-form :inline="true" :model="listQueryEqp" class="demo-form-inline demo-form-zdy">
          ...
          <el-form-item size="mini">
            <el-button type="primary" icon="el-icon-search" @click="onEquipQuery()">{{ $t('button.query') }}</el-button>
          </el-form-item>
        </el-form>
      </div>
