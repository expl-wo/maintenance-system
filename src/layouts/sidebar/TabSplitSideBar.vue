...
export default {
  ...
  methods: {
    ...
    getFullPath(item) {
      if (isExternal(item.pUrl)) return item.pUrl;
      return path.resolve(this.basePath, item.pUrl);
    },
    doChangeTab(route) {
    ...
    findPath(tab, fullPath) {
      ...
      if (firstItem.children && firstItem.children.length > 0) {
        this.findPath(firstItem, path.resolve(fullPath, firstitem.pUrl));
      } else {
        if (isExternal(firstitem.pUrl)) {
          window.open(firstitem.pUrl);
        } else {
          this.noChange = true;
          this.$router.push({ path: path.resolve(fullPath, firstitem.pUrl) });
        }
      }
    },
  },
